...
While watching a movie or series on Stremio, press the X-Ray button to reveal a sidebar showing the **cast currently on screen** and the **soundtrack** for what you're watching. Powered by TMDB and Spotify.
...
- **Soundtrack** — Browse the movie/series soundtrack with Spotify links
...
That's it. The installer downloads everything it needs automatically.
...
6. **Player UI** — On the player route, FIS adds an X-Ray button to the control bar. Clicking it opens a sidebar with cast and soundtrack data fetched from TMDB and Spotify.
...
|------|----------|---------|
| `feelit.bundle.js` | `%LOCALAPPDATA%\FIS\` | The JavaScript mod (downloaded from `feel-it.stream`) |
...
```bash
git clone https://github.com/MwoaA-a/FIS-Mod.git
cd FIS-Mod
npm install
npm run build
```
...
You can compare your locally built `feelit.bundle.js` against the one served at `https://feel-it.stream/stremio/feelit.bundle.js` to verify they are identical.
...
**Is this safe?**
The full source code is available in this repo. The injector only communicates with `localhost:9222` (Stremio's own debugging port). The bundle is served from `feel-it.stream` over HTTPS, or you can build it yourself.
